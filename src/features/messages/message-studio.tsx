...
type Saved = { id: string; content: string; category: string };
...
export function MessageStudio() {
  ...
  return (
    <div className="mx-auto max-w-3xl">
      <h1 className="font-display text-4xl text-navy">Messages</h1>
      <p className="mt-2 text-muted">Draft, save, and copy templates here. Daily Love cards stay in Daily Love.</p>
      ...
      {saved.length ? (
        <div className="mt-10 space-y-3">
          ...
          {saved.map((m) => (
            <Card key={m.id}>
              <p className="text-xs text-muted">{m.category.replaceAll("_", " ")}</p>
              <p className="mt-2 text-sm leading-6">{m.content}</p>
              <Button size="sm" variant="outline" className="mt-3" onClick={() => copyToClipboard(m.content).then(() => toast.success("Copied"))}>
                Copy
              </Button>
            </Card>
          ))}
